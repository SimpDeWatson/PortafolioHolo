  ...
  for (const rec of list) {
    ...
    if (rawUrl) {
      ...
      btnDl.addEventListener('click', (e) => {
        e.preventDefault();
        downloadFileBlob(rawUrl, rec.name, btnDl);
      });
...
/** Descarga real (blob). El atributo download no funciona en URLs cross-origin de GitHub. */
async function downloadFileBlob(url, fileName, btn) {
  ...
  try {
    ...
    console.warn('Descarga blob falló', err);
    window.open(url, '_blank', 'noopener');
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.textContent = label || '⬇️ Descargar';
    }
  }
}
...
function openFullView(viewUrl, downloadUrl, name, kind) {
  ...
  if (!overlay) {
      ...
      <div class="file-fullview-panel">
        <div class="file-fullview-bar">
          ...
          <div class="file-fullview-btns">
            ...
            <button type="button" class="btn-file-action btn-file-download" data-dl>⬇️ Descargar</button>
    ...
    const panelEl = overlay.querySelector('.file-fullview-panel');
  ...
  const fileName = name || 'archivo';
  const dlNew = dl.cloneNode(true);
  dl.parentNode.replaceChild(dlNew, dl);
  dlNew.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
    downloadFileBlob(fileUrl, fileName, dlNew);
  });
